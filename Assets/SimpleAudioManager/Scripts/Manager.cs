...
namespace SimpleAudioManager
{
    public class Manager : MonoBehaviour
    {
        ...
        [Tooltip("The audio source prefab which will be used in the audio source pool.")]
        ...
        [Tooltip("The maximum volume for the audio clips.")]
        [Range(0f, 1f)]
        public float maxVolume = 1f;

        [Tooltip(
            "The amount of time it will take for different songs to blend between one-another."
        )]
        ...
        public void PlaySong(PlaySongOptions pOptions)
        {
            ...
            if (_data == null || _data.Count == 0 || _data.Count <= pOptions.song)
                return;
            if (
                _data[pOptions.song].intensityClips == null
                || _data[pOptions.song].intensityClips.Count == 0
            )
                return;
            ...
            if (_currentSourceIndex != -1)
            {
                ...
                float _fadeTime =
                    (pOptions.blendOutTime == -1f)
                        ? _currentSongData.reverbTail
                        : pOptions.blendOutTime;
        ...
        /// </summary>
        private IEnumerator _FadeVolume(
            AudioSource pSource,
            float pStart,
            float pEnd,
            float pDuration
